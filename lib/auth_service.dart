...
// --- AuthService Class (Complete Implementation) ---

class AuthService {
  final _storage = const FlutterSecureStorage();
  
  // Base URLs - Choose the appropriate one for your setup
  static const String _baseUrl = 'http://10.74.47.55:3000/api'; // Use your PC's WiFi IP for physical device // For Android emulator
  // static const String _baseUrl = 'http://localhost:3000/api'; // For iOS simulator
  // static const String _baseUrl = 'http://192.168.1.100:3000/api'; // For physical device
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    ...
    try {
      ...
      if (response.body.trim().startsWith('<!DOCTYPE html>') || 
        ...
        return {
          'success': false, 
          'message': 'Backend server returned HTML instead of JSON. Check if server is running correctly.'
        };
      }
      ...
      return {
        'success': false, 
        'message': 'Network error: Cannot connect to server. Make sure backend is running. Details: $e'
      };
    } catch (e) {
      return {
        'success': false, 
  ...
  Future<Map<String, dynamic>> register(String name, String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        return {
          'success': false, 
          'message': data['message'] ?? 'Registration failed with status ${response.statusCode}'
        };
      }
    } on http.ClientException catch (e) {
      return {
  ...
  try {
    final response = await http.post(
      Uri.parse('$_authBaseUrl/register'), // or '/signup' depending on your backend
      headers: {'Content-Type': 'application/json'},
      body: json.encode({
        'name': name,
        'email': email,
    ...
    if (response.statusCode == 201) {
      ...
      return {
        'success': false,
        'message': data['message'] ?? 'Signup failed with status ${response.statusCode}',
      };
    }
  } catch (e) {
  ...
  // Add this method to AuthService class for debugging
Future<void> debugAuthStatus() async {
  ...
  Future<void> logout() async {
    try {
      ...
      // Even if there's an error, we should clear the storage
      await _storage.delete(key: _tokenKey);
      await _storage.delete(key: _userKey);
    }
  }
  ...
  // ✅ 5. GET CURRENT USER FROM SERVER
  ...
  Future<HealthMetrics> updateHealthMetric(String metric, dynamic value) async {
    try {
      ...
      final response = await http.patch(
        Uri.parse('$_baseUrl/health/metrics/$metric'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  // ✅ 7. PCOD/MENSTRUAL TRACKER METHODS
  Future<Map<String, dynamic>> getPCODData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$_baseUrl/pcod/tracker'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  Future<List<dynamic>> getHealthReports() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(data['message'] ?? 'Failed to fetch health reports');
      }
    } catch (e) {
      throw Exception('Health reports error: $e');
    }
  }

  Future<dynamic> createHealthReport(Map<String, dynamic> report) async {
    try {
      final token = await getToken();
      if (token == null) throw Exception('Not authenticated');
      ...
      throw Exception('Create health report error: $e');
    }
  }
}